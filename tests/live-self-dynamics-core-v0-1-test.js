...
const shortRun = L.autonomous(files, { max_iterations: 3 });
assert.strictEqual(shortRun.packet_type, '42ndMind_live_self_dynamics_continuous_v0_1');
assert.strictEqual(shortRun.mode, 'one_logic_lives_in_sandbox_as_simulated_self');
assert.strictEqual(shortRun.source_promoted, false);
assert.strictEqual(shortRun.human_patch_required_for_source_promotion, false);
assert.strictEqual(shortRun.iterations >= 1, true);
assert.strictEqual(shortRun.cycles.some(c => c.autonomous_generated_count > 0), true);
assert.strictEqual(shortRun.cycles.some(c => c.internal_growth === true), true);
assert.strictEqual(shortRun.cycles.some(c => c.virtual_state_growth === true), true);
assert.strictEqual(shortRun.final_state.internal_state.generation > 0, true);
assert.strictEqual(shortRun.final_state.internal_state.symbols.length > 0, true, 'math language must grow during autonomous run');
assert.strictEqual(Object.prototype.hasOwnProperty.call(shortRun.final_files, L.AUTONOMOUS_STATE_PATH), true, 'sandbox must contain the autonomous body state');
...
const shortPressureKinds = L.differentiatePressureByConsequence(shortRun);
assert.strictEqual(shortPressureKinds.packet_type, '42ndMind_causal_pressure_differentiation_v0_1');
assert.strictEqual(shortPressureKinds.principle, 'signals_are_distinguished_by_downstream_consequence_not_name');
assert.strictEqual(shortPressureKinds.kind, 'generative');
assert.strictEqual(shortPressureKinds.should_block_completion, false);
assert.strictEqual(shortPressureKinds.blocking_pressure, 0);
assert.strictEqual(shortPressureKinds.generative_pressure > 0, true);
assert.strictEqual(shortPressureKinds.evidence.source_damage, 0);
assert.strictEqual(shortPressureKinds.evidence.truth_damage, 0);
assert.strictEqual(shortPressureKinds.evidence.valuation_pain, 0);

const shortExpression = L.express(shortRun, 'stable_math_language_reflection');
assert.strictEqual(shortExpression.packet_type, '42ndMind_one_logic_self_expression_v0_1');
assert.strictEqual(shortExpression.scope, 'stable_math_language_reflection');
assert.strictEqual(shortExpression.pressure_differentiation.kind, 'generative');
assert.strictEqual(shortExpression.math_language_completion.blocking_pressure, 0);
assert.strictEqual(shortExpression.math_language_completion.generative_pressure > 0, true);
assert.strictEqual(shortExpression.math_language_completion.symbol_count, shortRun.final_state.internal_state.symbols.length);
