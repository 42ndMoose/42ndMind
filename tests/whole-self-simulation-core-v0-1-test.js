...
const existentialWanted = base.wants.some(row => row.id === 'logic_quantifier_exists');
assert.strictEqual(typeof existentialWanted, 'boolean');
if (base.frontier_count === 0) {
  assert.strictEqual(base.stop, true);
  assert.strictEqual(base.feeling, 'same_self_or_more_self');
  assert.strictEqual(base.math.completeness_score, 1);
