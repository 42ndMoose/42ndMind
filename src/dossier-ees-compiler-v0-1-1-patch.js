/* 42ndMind Dossier EES Compiler v0.1.1 patch
 * Patch over DossierEESCompilerV01.
 *
 * Fix:
 * v0.1.0 treated any command/intent language without source support as a hard issue.
 * That incorrectly blocked unresolved mechanism records that explicitly preserve the
 * command/intent question as unresolved instead of asserting it as fact.
 *
 * Rule:
 * - direct_coordination without source remains a hard issue.
 * - command/intent language without source is allowed when mechanism_class is
 *   unresolved_mechanism or unsupported_conspiracy_overclaim and the mechanism has
 *   unresolved_questions or overclaim_flags.
 * - allowed unresolved command/intent language is reported as a warning, not a blocker.
 */
(function (global) {
  'use strict';

  if (!global.DossierEESCompilerV01 || typeof global.DossierEESCompilerV01.validatePacket !== 'function') {
    global.DossierEESCompilerV011PatchStatus = { installed:false, reason:'DossierEESCompilerV01_missing' };
    return;
  }
  ...
  function hasCommandIntentLanguage(mech) {
    return /intent|motive|deliberate|command|ordered|coordinated/i.test(`${mech.label || ''} ${mech.scope_note || ''}`);
  }
  ...
  function validatePacket(packet = {}) {
    ...
    asArray(packet.mechanisms).forEach(mech => {
      ...
      if (hasCommandIntentLanguage(mech) && !hasSource) {
        if (isUnresolvedAllowed(mech)) warnings.push(`unresolved_intent_or_command_language_without_source:${id}`);
  ...
  global.DossierEESCompilerV011PatchStatus = { installed:true, version:VERSION };
