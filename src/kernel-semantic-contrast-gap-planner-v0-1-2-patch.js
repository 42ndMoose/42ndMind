/* 42ndMind Semantic Contrast Gap Planner v0.1.2 Patch
 * Adds closure/source/evidence templates for remaining contrast gaps such as
 * misleading, disputed, clip, summary, and hearsay.
 *
 * Planning only: no truth decision, belief movement, doctrine promotion, or source write.
 */
(function (global) {
  'use strict';
  ...
  const EXTRA_TEMPLATES = Object.freeze({
    misleading: {
      operator_name: 'misleading',
      group: 'closure_dismissal',
      priority_hint: 10,
      rationale: 'Misleading is weaker than false: it marks framing, omission, or scope distortion without requiring direct contradiction.',
      sentences: [
        'The claim is misleading because it omits the qualifying footnote.',
        'The headline is misleading even though the quoted number is real.'
      ]
    },
    disputed: {
      operator_name: 'disputed',
      group: 'closure_dismissal',
      priority_hint: 8,
      rationale: 'Disputed means contested; it must not collapse into debunked, false, or contradicted.',
      sentences: [
        'The claim is disputed by several reviewers.',
        'The policy result is disputed, but it has not been disproven.'
      ]
    },
    clip: {
      operator_name: 'clip',
  ...
  function doctrine() {
    ...
    d.patch_adds_remaining_gap_templates = true;
  ...
  function planFromDistillation(distillation, options = {}) {
    ...
    return {
      packet_type: base.PACKET_TYPE || '42ndMind_semantic_contrast_gap_planner_v0_1',
      packet_version: VERSION,
      patch_packet_type: '42ndMind_semantic_contrast_gap_planner_v0_1_2_patch',
      created_at: now(),
      ok: true,
