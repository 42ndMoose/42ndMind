/* 42ndMind Semantic Vector Template Planner v0.1.1 patch
 * Adds a natural sentence rule for accusation-risk pressure signatures.
 * The patch replaces planner methods without moving belief or promoting doctrine.
 */
(function (global) {
  'use strict';

  const base = global.KernelSemanticVectorTemplatePlannerV01;
  if (!base) throw new Error('KernelSemanticVectorTemplatePlannerV01 unavailable for v0.1.1 patch');

  const VERSION = '0.1.1';
  const PACKET_TYPE = '42ndMind_semantic_vector_template_planner_v0_1_1_patch';
  ...
  function doctrine() {
    return Object.assign({}, base.doctrine(), {
      ...
      patch_adds_accusation_risk_natural_template_sentences: true,
      generic_fallback_sentences_are_not_preferred_seed_candidates: true,
  ...
  function planFromVectorSpace(space, options = {}) {
    ...
    return {
      packet_type: '42ndMind_semantic_vector_template_plan_v0_1_1',
