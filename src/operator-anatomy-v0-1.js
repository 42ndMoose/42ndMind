(function(root, factory) {
  ...
  const CATALOG = Object.freeze({
    affine_equation: Object.freeze({
      ...
      inverse_chain: ['undo-offset', 'undo-coefficient'],
      closure_operator: 'solveAffineEquation',
      closure_result: 'variable_value',
      examples: ['2x + 1 = 7', '-3y - 6 = 9'],
      assertion: "assert.strictEqual(P.solveAffineEquation('2x + 1 = 7').value, 3);"
    }),
    affine_expression: Object.freeze({
      id: 'affine_expression',
      operation: 'decompose',
      surface: 'a*x + b',
      parts: ['coefficient', 'variable', 'offset'],
      preconditions: ['coefficient is finite', 'offset is finite'],
      inverse_chain: [],
      closure_operator: 'decomposeAffineExpression',
      closure_result: 'coefficient_variable_offset',
      examples: ['2x + 1', '-3y - 6'],
      assertion: "assert.strictEqual(P.decomposeAffineExpression('-3y - 6').coefficient, -3);"
    }),
    linear_relation_truth: Object.freeze({
      id: 'linear_relation_truth',
      operation: 'evaluate',
      surface: 'x >= n under x = v',
      parts: ['variable', 'relation', 'target', 'assignment'],
      preconditions: ['assignment is finite', 'target is finite'],
      inverse_chain: [],
      closure_operator: 'evaluateLinearRelation',
      closure_result: 'truth_value',
      examples: ['x >= 3 with x = 5'],
      assertion: "assert.strictEqual(P.evaluateLinearRelation({ relation: 'x >= 3', value: 5 }).truth, true);"
    }),
    division_constraint: Object.freeze({
      id: 'division_constraint',
      operation: 'guard',
      surface: 'x/y undefined when y = 0',
      parts: ['numerator', 'denominator'],
      preconditions: ['denominator != 0'],
      violations: ['denominator = 0'],
      inverse_chain: [],
      closure_operator: 'proveDivisionByZeroUndefined',
      closure_result: 'undefined',
      examples: ['x/y is undefined when y = 0'],
    ...
    square_nonnegative: Object.freeze({
      id: 'square_nonnegative',
      operation: 'prove',
      surface: 'x^2 >= 0 over reals',
      parts: ['base', 'product_of_same_term'],
      preconditions: ['base is real'],
      inverse_chain: [],
      closure_operator: 'proveSquareNonnegative',
      closure_result: 'nonnegative_truth',
      examples: ['∀x ∈ ℝ, x^2 >= 0'],
    ...
    statement_classification: Object.freeze({
      ...
      operation: 'classify',
      surface: 'compiled math packet to closure class',
      parts: ['mode', 'operators', 'relation', 'domain'],
      preconditions: ['packet has mode'],
      inverse_chain: [],
      closure_operator: 'classifyMathStatement',
      closure_result: 'closure_operator_name',
      examples: ['∀x ∈ ℝ, x^2 >= 0'],
  ...
  function catalog() {
    return Object.keys(CATALOG).sort().map(key => Object.assign({}, CATALOG[key]));
  }
  ...
  function pressure(parserSource, options) {
    ...
    return {
      packet_type: '42ndMind_operator_anatomy_pressure_v0_1',
      version: VERSION,
      scalar,
      surfaces,
      gaps: gaps.map(g => ({ needle: g.needle, anatomy_id: g.anatomy_id, operation: g.operation })),
      unit: {
        surfaces: R(surfaces.length ? 1 : 0),
        gaps: R(gaps.length ? 1 : 0),
        ok: true
      },
      ξ: ''
    };
