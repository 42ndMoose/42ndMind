...
function makeMarkdown(bundle){const lines=['# Auto Growth Report','',`decision: ${bundle.decision}`,`reason: ${bundle.decision_reason}`,`current: ${bundle.current_baseline}`,`next: ${bundle.proposed_next_baseline}`,`entries: ${bundle.proposed_entry_count}`,'','future preflight:'];const f=bundle.future_preflight_summary||{};lines.push(`- staged seed packet valid: ${f.staged_seed_packet_valid===true}`);lines.push(`- staged pressures registered: ${f.staged_pressures_registered===true}`);lines.push(`- temp combined corpus valid: ${f.temp_combined_corpus_valid===true}`);lines.push(`- temp vector compression ok: ${f.temp_vector_compression_ok===true}`);lines.push(`- temp combined entries: ${f.temp_combined_entry_count==null?'unknown':f.temp_combined_entry_count}`);lines.push(`- temp vectors: ${f.temp_vector_count==null?'unknown':f.temp_vector_count}`);lines.push(`- missing pressures: ${f.missing_pressure_count==null?'unknown':f.missing_pressure_count}`);lines.push('','gates:');bundle.gates.forEach(g=>lines.push(`- ${g.name}: ${g.status}`));lines.push('','checks:','- staged output only','- training pressure only','- not doctrine','- belief_movement: none');return lines.join('\n');}
function make(packet){const seed=clone(packet&&packet.seed_packet_draft||{});const bundle={packet_type:PACKET_TYPE,packet_version:VERSION,created_at:now(),ok:packet&&packet.decision==='AUTO_STAGE',decision:text(packet&&packet.decision),decision_reason:text(packet&&packet.decision_reason),current_baseline:text(packet&&packet.current_baseline),proposed_next_baseline:text(packet&&packet.proposed_next_baseline),proposed_entry_count:Number(packet&&packet.proposed_entry_count||0),gates:gateRows(packet),future_preflight_summary:clone(packet&&packet.future_preflight_summary||{}),staged_seed_packet:seed,staged_seed_json:JSON.stringify(seed,null,2),operators_in_seed:seedOperators(seed),pressures_in_seed:seedPressures(seed),markdown_report:'',doctrine:doctrine(),belief_movement:'none'};bundle.markdown_report=makeMarkdown(bundle);return bundle;}
