/* 42ndMind Semantic Operator Workbench v0.1.2 Patch
 * Suppresses redundant generic proof/falsity matches when the compound
 * proved_false(evidence,claim) operator is present.
 *
 * This prevents a single phrase like "proved the claim false" from overweighting
 * the same semantic pressure through proved(evidence,claim), false(claim), and
 * proved_false(evidence,claim) at once.
 *
...
(function (global) {
  ...
  const VERSION = '0.1.2';
  const PATCH_PACKET = '42ndMind_semantic_operator_workbench_v0_1_2_patch';
  ...
  function doctrine() {
    ...
    d.patch_suppresses_redundant_generic_proof_when_compound_proved_false_exists = true;
  ...
  function suppressRedundantMatches(matches) {
    ...
    const hasProvedFalse = list.some(m => m.name === 'proved_false');
  ...
  function lexicalAction(report) {
    ...
    if (has(report, 'works')) return 'define the outcome metric before accepting effectiveness pressure';
    if (has(report, 'reliable')) return 'treat reliability as source-history pressure, not claim truth';
    if (has(report, 'states') || has(report, 'deadline_extended')) return 'map the document passage to the factual update and check scope';
  ...
  function sourceTrustAction(pressures) {
    ...
    if (p.includes('source_trust_pressure') || p.includes('authority_transfer_pressure') || p.includes('trust_inflation_pressure') || p.includes('reviewer_status_pressure') || p.includes('rating_pressure') || p.includes('provenance_pressure')) return 'treat source/status/provenance labels as metadata or prior pressure, not truth';
  ...
  function questionsForReport(report) {
    const q = [];
    if (has(report, 'proved_false')) q.push('What exact evidence directly falsifies the exact claim under the same definitions and scope?');
    if (has(report, 'certified')) q.push('What does the certification cover, and is it being substituted for claim-level evidence?');
    if (has(report, 'works')) q.push('What outcome metric, baseline, timeframe, and comparison class show that the policy works?');
    if (has(report, 'debate_over')) q.push('What exact debate is being closed, and what unresolved questions remain?');
    if (has(report, 'reliable')) q.push('What source-history evidence supports reliability, and what claim-level evidence supports this claim?');
    if (has(report, 'posted')) q.push('Does posting establish only provenance, or does the post include supporting evidence?');
    if (has(report, 'states') || has(report, 'deadline_extended')) q.push('Which exact passage states the update, and does it apply to this case?');
    if (has(report, 'published_summary')) q.push('Does the summary preserve the underlying raw data definitions and scope?');
    if (has(report, 'raw_data')) q.push('What calculation or record in the raw data contradicts the summary?');
