(function(root, factory) {
  ...
  function obligationDefs(options) {
    const p = proofContract(options || {});
    const rows = O(p.obligations);
    const order = A(p.order);
    const ids = order.length ? order : Object.keys(rows).sort();
    const ph = phase(options || {});
    return ids.map(id => Object.assign({ id }, O(rows[id]))).filter(row => row.phase === 'both' || row.phase === ph || !row.phase);
  }

  function checkRow(row, options) {
    const F = formulas(options || {});
    const e = evidence(options || {});
    const missing_required = A(row.requires).filter(f => F.indexOf(f) < 0);
    ...
    const ok = missing_required.length === 0 && missing_theorem.length === 0 && failed_checks.length === 0;
  ...
  function buildProofObligations(state, transition, options) {
    const o = Object.assign({}, options || {}, { state, transition });
    return obligationDefs(o).map(row => checkRow(row, o));
  }

  function certificate(rows, options) {
    const p = proofContract(options || {});
    const failed = rows.find(row => !row.ok) || null;
  ...
  function missingContract(options) {
    return certificate([{ id: 'ProofContract', ok: false, formula: null, requires: ['CONTRACT.proofs'], checks: [], result: null, missing_required: ['CONTRACT.proofs'], missing_theorem: [], failed_checks: [] }], options || {});
  }

  function checkProof(proof, options) {
    if (!proofContract(options || {}).obligations) return missingContract(options || {});
    const p = O(proof);
    if (A(p.obligations).length) return certificate(A(p.obligations).map(row => checkRow(row, Object.assign({}, options || {}, { evidence: p.evidence || evidence(options || {}) }))), options || {});
    return certificate(buildProofObligations(null, null, options || {}), options || {});
  }

  function proveState(state, options) {
    const o = Object.assign({}, options || {}, { phase: 'state' });
    if (!proofContract(o).obligations) return missingContract(o);
    return certificate(buildProofObligations(state, null, o), o);
  }

  function proveTransition(before, candidate, after, options) {
    const o = Object.assign({}, options || {}, { phase: 'transition', before, candidate, after });
    if (!proofContract(o).obligations) return missingContract(o);
    return certificate(buildProofObligations(after || before, evidence(o), o), o);
  }
