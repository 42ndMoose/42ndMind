/* 42ndMind Semantic Operator Workbench v0.1.4 Patch
 * Adds draft-language support for scope/qualification contrast operators:
...
(function (global) {
  ...
  const VERSION = '0.1.4';
  const PATCH_PACKET = '42ndMind_semantic_operator_workbench_v0_1_4_patch';
  ...
  function doctrine() {
    ...
    d.patch_adds_scope_qualification_draft_language = true;
  ...
  function suppressRedundantMatches(matches, sentence) {
    ...
    return list.filter(m => {
      ...
      if (names.includes('dataset') && m.name === 'raw_data' && raw.includes('dataset')) return false;
      return true;
    });
  }
  ...
  function intendedMeaning(report) {
    ...
    if (names.includes('different_scope')) return 'The sentence points to a scope mismatch that may explain apparent contradiction without making either side false.';
    if (names.includes('qualifies')) return 'The sentence qualifies or limits a claim without turning that qualification into direct contradiction.';
    if (names.includes('narrows')) return 'The sentence narrows a broader claim into a bounded claim with a smaller scope.';
    if (names.includes('footnote')) return 'The sentence uses a footnote as qualifying evidence that must be mapped to the exact claim.';
    if (names.includes('dataset')) return 'The sentence uses dataset contact and requires scope, definition, and aggregation checks.';
  ...
  function lexicalAction(report) {
    if (has(report, 'different_scope')) return 'compare record scope and claim scope before treating the mismatch as contradiction';
    if (has(report, 'qualifies')) return 'preserve qualification pressure and avoid false contradiction closure';
    if (has(report, 'narrows')) return 'convert broad claim pressure into bounded/narrowed claim pressure';
    if (has(report, 'footnote')) return 'map the footnote to the exact claim and identify whether it narrows, defines, or only comments';
    if (has(report, 'dataset')) return 'check dataset definitions, scope, exclusions, cleaning, and aggregation level';
  ...
  function sourceTrustAction(pressures) {
    ...
    if (p.includes('evidence_contact_pressure') || p.includes('scope_mismatch_pressure') || p.includes('qualification_pressure') || p.includes('narrowing_pressure')) return 'inspect the evidence object and scope relation before support/contradiction movement';
    return 'not source-driven unless sources are later attached';
  }

  function questionsForReport(report) {
    const q = [];
    if (has(report, 'different_scope')) q.push('Do the record and claim differ by date, geography, population, definition, or aggregation level?');
    if (has(report, 'qualifies')) q.push('What exactly is narrowed, limited, or conditioned by the qualifying evidence?');
    if (has(report, 'narrows')) q.push('What broader claim is being narrowed, and what bounded version remains supported?');
    if (has(report, 'footnote')) q.push('Does the footnote directly limit the claim, define a term, or merely add commentary?');
    if (has(report, 'dataset')) q.push('What are the dataset scope, definitions, exclusions, and aggregation level?');
